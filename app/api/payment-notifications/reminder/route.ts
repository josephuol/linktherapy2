...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (payment.status === "completed") {
      console.log("[Payment Notification] Payment already completed, skipping reminder")
    ...
    if (payment.commission_amount <= 0) {
      console.log("[Payment Notification] Commission amount is 0 or less, skipping reminder")
    ...
    const paymentDueDate = new Date(payment.payment_due_date)
    ...
    return NextResponse.json({ ok: true, message: "Reminder email sent successfully" })
  } catch (error: any) {
